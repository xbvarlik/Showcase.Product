"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var dist_exports = {};
__export(dist_exports, {
  BinaryType: () => import_chunk_FPHZVRV4.BinaryType,
  ClientEngineType: () => import_chunk_CKNTB45N.ClientEngineType,
  DEFAULT_CLIENT_ENGINE_TYPE: () => import_chunk_CKNTB45N.DEFAULT_CLIENT_ENGINE_TYPE,
  ErrorArea: () => import_chunk_A4QWQE3W.ErrorArea,
  Generator: () => import_chunk_635TMBAP.Generator,
  HelpError: () => import_chunk_6JE6QGSJ.HelpError,
  RustPanic: () => import_chunk_A4QWQE3W.RustPanic,
  SchemaEngineExitCode: () => import_chunk_4RWCRJYD.SchemaEngineExitCode,
  arg: () => import_chunk_MUPZUVSI.arg,
  assertAlways: () => import_chunk_Y4TOGY6I.assertAlways,
  assertNever: () => import_chunk_IL7PSY44.assertNever,
  binaryTargetRegex: () => import_chunk_AOQIUJT4.binaryTargetRegex,
  byline: () => import_chunk_L6KOIJJ5.byline,
  callOnceOnSuccess: () => import_chunk_NIWBAJZV.callOnceOnSuccess,
  canConnectToDatabase: () => import_chunk_4RWCRJYD.canConnectToDatabase,
  canPrompt: () => import_chunk_EPJYKNCQ.canPrompt,
  checkUnsupportedDataProxy: () => import_chunk_PVHNT4K2.checkUnsupportedDataProxy,
  chmodPlusX: () => import_chunk_TWSLDIFL.chmodPlusX,
  createDatabase: () => import_chunk_4RWCRJYD.createDatabase,
  credentialsToUri: () => import_chunk_X4U4GLWH.credentialsToUri,
  debugMultipleSchemaPaths: () => import_chunk_YRBRUREX.debugMultipleSchemaPaths,
  debugMultipleSchemas: () => import_chunk_YRBRUREX.debugMultipleSchemas,
  drawBox: () => import_chunk_F3SIWK3J.drawBox,
  dropDatabase: () => import_chunk_4RWCRJYD.dropDatabase,
  engineEnvVarMap: () => import_chunk_FPHZVRV4.engineEnvVarMap,
  extractPreviewFeatures: () => import_chunk_TSJICM5S.extractPreviewFeatures,
  extractSchemaContent: () => import_chunk_YRBRUREX.extractSchemaContent,
  fixBinaryTargets: () => import_chunk_X77CS5RK.fixBinaryTargets,
  format: () => import_chunk_MUPZUVSI.format,
  formatSchema: () => import_chunk_PVHNT4K2.formatSchema,
  formatTable: () => import_chunk_QN6PSQY7.formatTable,
  formatms: () => import_chunk_XYCIH36R.formatms,
  fsFunctional: () => import_chunk_S3323RUL.fs_functional_exports,
  fsUtils: () => import_chunk_E3APO3ZG.fs_utils_exports,
  getBinaryTargetForCurrentPlatform: () => import_chunk_PVHNT4K2.getBinaryTargetForCurrentPlatform,
  getCLIPathHash: () => import_chunk_PVHNT4K2.getCLIPathHash,
  getClientEngineType: () => import_chunk_CKNTB45N.getClientEngineType,
  getCommandWithExecutor: () => import_chunk_ZA3RMW2P.getCommandWithExecutor,
  getConfig: () => import_chunk_PI3AAZEE.getConfig,
  getDMMF: () => import_chunk_EIPWZ4J5.getDMMF,
  getDirectUrl: () => import_chunk_PI3AAZEE.getDirectUrl,
  getEffectiveUrl: () => import_chunk_PI3AAZEE.getEffectiveUrl,
  getEngineVersion: () => import_chunk_G7LL2H2O.getEngineVersion,
  getEnginesMetaInfo: () => import_chunk_JMZHUHCC.getEnginesMetaInfo,
  getEnvPaths: () => import_chunk_PVHNT4K2.getEnvPaths,
  getGenerator: () => import_chunk_PVHNT4K2.getGenerator,
  getGeneratorSuccessMessage: () => import_chunk_P6VBFLWR.getGeneratorSuccessMessage,
  getGenerators: () => import_chunk_PVHNT4K2.getGenerators,
  getLintWarningsAsText: () => import_chunk_KJGE5YOE.getLintWarningsAsText,
  getMigrateConfigDir: () => import_chunk_RFB2U4GQ.getMigrateConfigDir,
  getNodeAPIName: () => import_chunk_PVHNT4K2.getNodeAPIName,
  getPackageCmd: () => import_chunk_3U2ANLD3.getPackageCmd,
  getPackedPackage: () => import_chunk_J3JE5NVO.getPackedPackage,
  getPrismaConfigFromPackageJson: () => import_chunk_PVHNT4K2.getPrismaConfigFromPackageJson,
  getProjectHash: () => import_chunk_PVHNT4K2.getProjectHash,
  getSchema: () => import_chunk_PVHNT4K2.getSchema,
  getSchemaDir: () => import_chunk_PVHNT4K2.getSchemaDir,
  getSchemaWithPath: () => import_chunk_PVHNT4K2.getSchemaWithPath,
  getSchemaWithPathOptional: () => import_chunk_PVHNT4K2.getSchemaWithPathOptional,
  handleLibraryLoadingErrors: () => import_chunk_J4MA2JIK.handleLibraryLoadingErrors,
  handleLintPanic: () => import_chunk_KJGE5YOE.handleLintPanic,
  handlePanic: () => import_chunk_PVHNT4K2.handlePanic,
  hasOwnProperty: () => import_chunk_WV4QHCSM.hasOwnProperty,
  highlightDatamodel: () => import_chunk_TQCLUNM2.highlightDatamodel,
  highlightSql: () => import_chunk_TQCLUNM2.highlightSql,
  highlightTS: () => import_chunk_TQCLUNM2.highlightTS,
  isCi: () => import_chunk_H4NI2RIK.isCi,
  isCurrentBinInstalledGlobally: () => import_chunk_5I4BOTBV.isCurrentBinInstalledGlobally,
  isError: () => import_chunk_MUPZUVSI.isError,
  isInteractive: () => import_chunk_ICFLO5JW.isInteractive,
  isPromiseLike: () => import_chunk_JTKAT4KZ.isPromiseLike,
  isRustPanic: () => import_chunk_A4QWQE3W.isRustPanic,
  isValidJsIdentifier: () => import_chunk_FHXAG4A2.isValidJsIdentifier,
  keyBy: () => import_chunk_ZKWDIAVF.keyBy,
  link: () => import_chunk_L6GBFCXG.link,
  lintSchema: () => import_chunk_KJGE5YOE.lintSchema,
  load: () => import_chunk_S4LINK5Y.loadLibrary,
  loadEnvFile: () => import_chunk_PVHNT4K2.loadEnvFile,
  locateLocalCloudflareD1: () => import_chunk_OSOXVAZB.locateLocalCloudflareD1,
  logger: () => import_chunk_AZNLB67U.logger_exports,
  longestCommonPathPrefix: () => import_chunk_J77ZIDXB.longestCommonPathPrefix,
  mapObjectValues: () => import_chunk_UGR6KLZN.mapObjectValues,
  maskSchema: () => import_chunk_KDHESRKE.maskSchema,
  maxBy: () => import_chunk_CGFNDGGI.maxBy,
  maxWithComparator: () => import_chunk_CGFNDGGI.maxWithComparator,
  mergeSchemas: () => import_chunk_RGGBGYAL.mergeSchemas,
  missingGeneratorMessage: () => import_chunk_2MWIMEFK.missingGeneratorMessage,
  parseAWSNodejsRuntimeEnvVarVersion: () => import_chunk_2SDPW6AX.parseAWSNodejsRuntimeEnvVarVersion,
  parseBinaryTargetsEnvValue: () => import_chunk_IOIAK7V7.parseBinaryTargetsEnvValue,
  parseEnvValue: () => import_chunk_IOIAK7V7.parseEnvValue,
  pathToPosix: () => import_chunk_J77ZIDXB.pathToPosix,
  pick: () => import_chunk_5V36RLCO.pick,
  printConfigWarnings: () => import_chunk_5FJ3MENK.printConfigWarnings,
  printGeneratorConfig: () => import_chunk_ONHKZPSV.printGeneratorConfig,
  protocolToConnectorType: () => import_chunk_X4U4GLWH.protocolToConnectorType,
  relativizePathInPSLError: () => import_chunk_WMIBAVWZ.relativizePathInPSLError,
  resolveBinary: () => import_chunk_FPHZVRV4.resolveBinary,
  resolveUrl: () => import_chunk_PI3AAZEE.resolveUrl,
  sendPanic: () => import_chunk_PVHNT4K2.sendPanic,
  serializeQueryEngineName: () => import_chunk_HO5HRHFZ.serializeQueryEngineName,
  setClassName: () => import_chunk_ANU3HEGF.setClassName,
  toMultipleSchemas: () => import_chunk_YRBRUREX.toMultipleSchemas,
  toSchemasContainer: () => import_chunk_JHQHVXIR.toSchemasContainer,
  toSchemasWithConfigDir: () => import_chunk_JHQHVXIR.toSchemasWithConfigDir,
  tryLoadEnvs: () => import_chunk_HMGEWJDY.tryLoadEnvs,
  unknownCommand: () => import_chunk_6JE6QGSJ.unknownCommand,
  uriToCredentials: () => import_chunk_X4U4GLWH.uriToCredentials,
  validate: () => import_chunk_JNP5TBS7.validate,
  vercelPkgPathRegex: () => import_chunk_M3GBKLGE.vercelPkgPathRegex,
  version: () => import_chunk_XZU52V3Q.version,
  warnOnce: () => import_chunk_UNUCCAXA.warnOnce,
  wasm: () => import_chunk_5JDKBNDH.wasm_exports
});
module.exports = __toCommonJS(dist_exports);
var import_chunk_PVHNT4K2 = require("./chunk-PVHNT4K2.js");
var import_chunk_IABMG3F4 = require("./chunk-IABMG3F4.js");
var import_chunk_3U2ANLD3 = require("./chunk-3U2ANLD3.js");
var import_chunk_FTS2IEWX = require("./chunk-FTS2IEWX.js");
var import_chunk_YDZVEVUG = require("./chunk-YDZVEVUG.js");
var import_chunk_EUTJYQDW = require("./chunk-EUTJYQDW.js");
var import_chunk_IPKE37QC = require("./chunk-IPKE37QC.js");
var import_chunk_KEBDP2C3 = require("./chunk-KEBDP2C3.js");
var import_chunk_X77CS5RK = require("./chunk-X77CS5RK.js");
var import_chunk_D4MCOTL5 = require("./chunk-D4MCOTL5.js");
var import_chunk_RS2R7COT = require("./chunk-RS2R7COT.js");
var import_chunk_4CGPN5SR = require("./chunk-4CGPN5SR.js");
var import_chunk_X3P4J7WW = require("./chunk-X3P4J7WW.js");
var import_chunk_ONHKZPSV = require("./chunk-ONHKZPSV.js");
var import_chunk_5V36RLCO = require("./chunk-5V36RLCO.js");
var import_chunk_5FJ3MENK = require("./chunk-5FJ3MENK.js");
var import_chunk_HO5HRHFZ = require("./chunk-HO5HRHFZ.js");
var import_chunk_ANU3HEGF = require("./chunk-ANU3HEGF.js");
var import_chunk_JHQHVXIR = require("./chunk-JHQHVXIR.js");
var import_chunk_HMGEWJDY = require("./chunk-HMGEWJDY.js");
var import_chunk_BGFWFIPR = require("./chunk-BGFWFIPR.js");
var import_chunk_2MWIMEFK = require("./chunk-2MWIMEFK.js");
var import_chunk_2SDPW6AX = require("./chunk-2SDPW6AX.js");
var import_chunk_L6GBFCXG = require("./chunk-L6GBFCXG.js");
var import_chunk_7B5PXS6N = require("./chunk-7B5PXS6N.js");
var import_chunk_UGR6KLZN = require("./chunk-UGR6KLZN.js");
var import_chunk_KDHESRKE = require("./chunk-KDHESRKE.js");
var import_chunk_CGFNDGGI = require("./chunk-CGFNDGGI.js");
var import_chunk_WV4QHCSM = require("./chunk-WV4QHCSM.js");
var import_chunk_JTKAT4KZ = require("./chunk-JTKAT4KZ.js");
var import_chunk_FHXAG4A2 = require("./chunk-FHXAG4A2.js");
var import_chunk_ZKWDIAVF = require("./chunk-ZKWDIAVF.js");
var import_chunk_S3323RUL = require("./chunk-S3323RUL.js");
var import_chunk_E3APO3ZG = require("./chunk-E3APO3ZG.js");
var import_chunk_J77ZIDXB = require("./chunk-J77ZIDXB.js");
var import_chunk_ZA3RMW2P = require("./chunk-ZA3RMW2P.js");
var import_chunk_5I4BOTBV = require("./chunk-5I4BOTBV.js");
var import_chunk_XLYQ6PQW = require("./chunk-XLYQ6PQW.js");
var import_chunk_RFB2U4GQ = require("./chunk-RFB2U4GQ.js");
var import_chunk_EPJYKNCQ = require("./chunk-EPJYKNCQ.js");
var import_chunk_H4NI2RIK = require("./chunk-H4NI2RIK.js");
var import_chunk_ICFLO5JW = require("./chunk-ICFLO5JW.js");
var import_chunk_OSOXVAZB = require("./chunk-OSOXVAZB.js");
var import_chunk_F3SIWK3J = require("./chunk-F3SIWK3J.js");
var import_chunk_ODI3VNBD = require("./chunk-ODI3VNBD.js");
var import_chunk_TSJICM5S = require("./chunk-TSJICM5S.js");
var import_chunk_HW5RCZSE = require("./chunk-HW5RCZSE.js");
var import_chunk_Y4TOGY6I = require("./chunk-Y4TOGY6I.js");
var import_chunk_IL7PSY44 = require("./chunk-IL7PSY44.js");
var import_chunk_AOQIUJT4 = require("./chunk-AOQIUJT4.js");
var import_chunk_L6KOIJJ5 = require("./chunk-L6KOIJJ5.js");
var import_chunk_NIWBAJZV = require("./chunk-NIWBAJZV.js");
var import_chunk_KJGE5YOE = require("./chunk-KJGE5YOE.js");
var import_chunk_RGGBGYAL = require("./chunk-RGGBGYAL.js");
var import_chunk_JNP5TBS7 = require("./chunk-JNP5TBS7.js");
var import_chunk_TQCLUNM2 = require("./chunk-TQCLUNM2.js");
var import_chunk_OIVPX3BR = require("./chunk-OIVPX3BR.js");
var import_chunk_CYAS6B5N = require("./chunk-CYAS6B5N.js");
var import_chunk_HPVMQFIY = require("./chunk-HPVMQFIY.js");
var import_chunk_E23HYCQJ = require("./chunk-E23HYCQJ.js");
var import_chunk_PI3AAZEE = require("./chunk-PI3AAZEE.js");
var import_chunk_EIPWZ4J5 = require("./chunk-EIPWZ4J5.js");
var import_chunk_YRBRUREX = require("./chunk-YRBRUREX.js");
var import_chunk_IAEMHQAC = require("./chunk-IAEMHQAC.js");
var import_chunk_WMIBAVWZ = require("./chunk-WMIBAVWZ.js");
var import_chunk_V6E4PZFJ = require("./chunk-V6E4PZFJ.js");
var import_chunk_XZU52V3Q = require("./chunk-XZU52V3Q.js");
var import_chunk_QN6PSQY7 = require("./chunk-QN6PSQY7.js");
var import_chunk_JMZHUHCC = require("./chunk-JMZHUHCC.js");
var import_chunk_PFLLVWZX = require("./chunk-PFLLVWZX.js");
var import_chunk_G7LL2H2O = require("./chunk-G7LL2H2O.js");
var import_chunk_S4LINK5Y = require("./chunk-S4LINK5Y.js");
var import_chunk_J4MA2JIK = require("./chunk-J4MA2JIK.js");
var import_chunk_H3T4L34H = require("./chunk-H3T4L34H.js");
var import_chunk_5JDKBNDH = require("./chunk-5JDKBNDH.js");
var import_chunk_5MUVEHBA = require("./chunk-5MUVEHBA.js");
var import_chunk_6JE6QGSJ = require("./chunk-6JE6QGSJ.js");
var import_chunk_P6VBFLWR = require("./chunk-P6VBFLWR.js");
var import_chunk_XYCIH36R = require("./chunk-XYCIH36R.js");
var import_chunk_CKNTB45N = require("./chunk-CKNTB45N.js");
var import_chunk_MUPZUVSI = require("./chunk-MUPZUVSI.js");
var import_chunk_A4QWQE3W = require("./chunk-A4QWQE3W.js");
var import_chunk_2B26F5N3 = require("./chunk-2B26F5N3.js");
var import_chunk_4RWCRJYD = require("./chunk-4RWCRJYD.js");
var import_chunk_7XNZDY2V = require("./chunk-7XNZDY2V.js");
var import_chunk_FPHZVRV4 = require("./chunk-FPHZVRV4.js");
var import_chunk_M3GBKLGE = require("./chunk-M3GBKLGE.js");
var import_chunk_RQPDQSHK = require("./chunk-RQPDQSHK.js");
var import_chunk_TWSLDIFL = require("./chunk-TWSLDIFL.js");
var import_chunk_7MD6344A = require("./chunk-7MD6344A.js");
var import_chunk_UNUCCAXA = require("./chunk-UNUCCAXA.js");
var import_chunk_635TMBAP = require("./chunk-635TMBAP.js");
var import_chunk_IOIAK7V7 = require("./chunk-IOIAK7V7.js");
var import_chunk_HOZFIXMD = require("./chunk-HOZFIXMD.js");
var import_chunk_X4U4GLWH = require("./chunk-X4U4GLWH.js");
var import_chunk_T2PM5TKT = require("./chunk-T2PM5TKT.js");
var import_chunk_4NFLAUKD = require("./chunk-4NFLAUKD.js");
var import_chunk_WCRKL55L = require("./chunk-WCRKL55L.js");
var import_chunk_J3JE5NVO = require("./chunk-J3JE5NVO.js");
var import_chunk_FUUQ6FJ4 = require("./chunk-FUUQ6FJ4.js");
var import_chunk_LKTTJWFT = require("./chunk-LKTTJWFT.js");
var import_chunk_T5RACBRF = require("./chunk-T5RACBRF.js");
var import_chunk_KKMREGHN = require("./chunk-KKMREGHN.js");
var import_chunk_4E7A3GDU = require("./chunk-4E7A3GDU.js");
var import_chunk_TZCE3PWZ = require("./chunk-TZCE3PWZ.js");
var import_chunk_H4J2FN2W = require("./chunk-H4J2FN2W.js");
var import_chunk_AZNLB67U = require("./chunk-AZNLB67U.js");
var import_chunk_PG5FDKSF = require("./chunk-PG5FDKSF.js");
var import_chunk_WWAWV7DQ = require("./chunk-WWAWV7DQ.js");
