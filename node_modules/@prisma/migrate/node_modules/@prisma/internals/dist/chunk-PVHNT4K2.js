...
var chunk_PVHNT4K2_exports = {};
__export2(chunk_PVHNT4K2_exports, {
...
module.exports = __toCommonJS2(chunk_PVHNT4K2_exports);
...
async function readSchemaFromDirectory(schemaPath2) {
  ...
  const config2 = await (0, import_chunk_PI3AAZEE.getConfig)({
    datamodel: files,
...
async function checkUnsupportedDataProxyMessage(command, args, implicitSchema) {
  ...
  for (const [argName, argValue] of argList) {
    ...
    if (argName.includes("schema")) {
      ...
      const config2 = await (0, import_chunk_PI3AAZEE.getConfig)({ datamodel, ignoreEnvVarErrors: true });
      const url = (0, import_chunk_PI3AAZEE.resolveUrl)((0, import_chunk_PI3AAZEE.getEffectiveUrl)(config2.datasources[0]));
...
async function getGenerators(options) {
  ...
  const config = await (0, import_chunk_PI3AAZEE.getConfig)({
    datamodel: schemas,
  ...
  const dmmf = await (0, import_chunk_EIPWZ4J5.getDMMF)({
    datamodel: schemas,
    datamodelPath: schemaPath,
    prismaPath,
    previewFeatures
...
The generator needs to either define the \`defaultOutput\` path in the manifest or you need to define \`output\` in the datamodel.prisma file.`
        ...
        const datamodel = (0, import_chunk_RGGBGYAL.mergeSchemas)({ schemas });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_EIPWZ4J5.getDMMF)({
            datamodel: schemas,
            datamodelPath: schemaPath,
...
async function formatSchema({ schemas: schemas2 }, inputFormattingOptions) {
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(
    () => {
      ...
      const lintDiagnostics2 = (0, import_chunk_KJGE5YOE.lintSchema)({ schemas: formattedMultipleSchemas2 });
      return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
    },
    { schemas: schemas2 }
  );
  const lintWarnings = (0, import_chunk_KJGE5YOE.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_5JDKBNDH.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
